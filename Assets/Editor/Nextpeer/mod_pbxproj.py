...
class XCBuildConfiguration(PBXType):
	def add_search_paths(self, paths, base, key, recursive=True, escape=True):
		...
		for path in paths:
			...
			if escape and path != "$(inherited)": # Patch used for framework paths. Escaping inherited paths potentially means escaping already escaped paths, which confuses Xcode apparently.
